...
export default {
  ...
  // computed: {
  //   getStatus () {
  //     return this.$store.state.comloginStatus
  //   }
  // },
  // watch: {
  //   getStatus (val) {
  //     console.log(val)
  //     console.log(this.$store.state.comloginStatus)
  //     if (!this.$store.state.comloginStatus) this.dialogVisible = true
  //   }
  // },
  mounted () {
    // 是否登录
    ...
    setTimeout(function () {
      if (QC.Login.check()) {
        ...
        _this.qqimg = storage.get('qqimg')
        _this.qqnick = storage.get('qqnick')
      }
    }, 200)
    this.$store.on('eventlogin', () => {
      this.loginIn()
    })
  },
  methods: {
    ...
    qqLogin () {
      ...
      QC.api('get_user_info', {}).success(
        function (s) {
          _this.qqimg = s.data.figureurl_qq_1
          _this.qqnick = s.data.nickname
          storage.set('qqimg', _this.qqimg)
          storage.set('qqnick', _this.qqnick)
          userinfo = {
            username: s.data.qqnick,
            figureurl: s.data.qqimg,
          ...
          if (QC.Login.check()) {
            ...
            QC.Login.getMe(function (openId, accessToken) {
              ...
              _this.$store.commit('registerUser', userinfo)
            })
          }
        }
      )
